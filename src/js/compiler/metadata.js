...
{
	const is_in_editor = (typeof editor_tabmanager !== 'undefined') // TODO: remove the need for that

	for (const [i, key] of state.metadata_keys.entries())
	{
		if ( is_in_editor && ! (key in metadata_in_tab) ) // WIP TODO: we should only do that in the editor, not in the player!
		{
